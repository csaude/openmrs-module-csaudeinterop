...
@Transactional
@Service
public class PrescriptionResponseProcessorServiceImpl implements PrescriptionResponseProcessorService {
	
	private static final Logger log = LoggerFactory.getLogger(PrescriptionResponseProcessorServiceImpl.class);
	...
	@Autowired
	public PrescriptionResponseProcessorServiceImpl(EncounterService encounterService, ConceptService conceptService,
	...
	@Override
	public void process(PrescriptionResponsePayload payload) {
		
		Encounter encounter = encounterService.getEncounterByUuid(payload.getPrescriptionUuid());
		if (encounter == null) {
			log.warn("Encounter not found for UUID: " + payload.getPrescriptionUuid());
			return;
		}
